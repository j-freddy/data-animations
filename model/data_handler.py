...
class DataHandler:
    ...
    def get_lerped_max(self, entry_index: float):
        # TODO Magic numbers
        return Lerp.weighted_avg(self.maxes, entry_index, kernel_size=29)

    def get_units_indices(self):
        units_indices = np.empty(self.num_entries())

        for i in range(self.num_entries()):
            lerped_max = self.get_lerped_max(i)

            # Choose appropriate unit
            min_ticks = 3
            # Choose largest unit that is not too large (i.e. shows at least
            # @min_ticks ticks)
            units_indices[i] = np.where((self.available_units * min_ticks) > lerped_max)[0][0] - 1
